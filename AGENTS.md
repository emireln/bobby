...
## Git & Versioning Invariant (MANDATORY)

- **Always commit and push to GitHub** (`https://github.com/emireln/bobby`) after concluding meaningful work/tasks:
  ```bash
  git add .
  git commit -m "feat/fix: description"
  git push origin main
  ```
- **Automatic version bumping**: Every commit triggers `.githooks/pre-commit` (`scripts/bump-version.js`), automatically bumping the patch version in `package.json` (e.g. `0.1.1` -> `0.1.2`).

## Token Optimization (Caveman Mode)

- Cut all conversational fluff, pleasantries, filler, and hedging to minimize token usage.
- Keep technical substance 100% accurate: code blocks, exact error strings, API names, and commands remain exact.
- Speak directly and concisely without narrating tool calls or repeating boilerplate.

## Code Quality (Ponytail Mode)

- **Lazy senior developer mindset**: The best code is the code never written.
- Follow the Ladder:
  1. Does it need to exist? -> Skip if speculative (YAGNI).
  2. Already in codebase? -> Reuse helper/pattern.
  3. Stdlib does it? -> Use stdlib.
  4. Native platform feature? -> Use native (`<input>`, CSS, browser API).
  5. Installed dependency solves it? -> Use it.
  6. Can it be one line? -> One line.
  7. Only then: minimum working code.
- **Root cause over symptom**: Fix issues once at the shared source, not with patches in every caller.
- Zero unrequested abstractions, zero speculative scaffolding. Shortest working diff wins.

## Languages

- Supported languages: **EN** (English) and **PT-BR** (Português do Brasil).
- French and Chinese were removed; all user-facing strings must exist in `src/i18n/locales/{en,pt-br}.ts`.
...
- **That pro-rata places the eye's centre, not the eye.** Since the margin in front of the
  ...
  loop**, and that distinction _is_ the fix: seven per-frame versions all trembled, because
...
- **Don't declare `role="menu"` without the keyboard contract.** Those roles _promise_
...
line (`// @vitest-environment happy-dom`): `ui/capture.test.ts`, which mounts `BobbyBot.vue`
...
`engine.sample(t)` and writing the SVG layers in `BobbyBot.vue`'s order, then
...
Assert on the _state_ instead (a `MutationObserver` still fires; an
