...
A paper-coloured `<path>` sits under the body (`BobbyBot.vue`), so the exported image
...
**The drawing is not rebuilt.** `svgAnime` takes the SVG `BobbyBot` already rendered for
...
be wrong. Hence `rendAt(t)` on `BobbyBot`: it resolves the block with `blockAt` and calls
