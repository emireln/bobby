...
/** Rayon de la boule au repos, cf. le `R` de BobbyBot.vue. */

/** Demi-cote du viewBox affiche a l'ecran, cf. le `VB` de BobbyBot.vue. */
const VB_ECRAN = 158
...
describe('nom de fichier', () => {
  it('se construit sur les ids et pas sur les libelles', () => {
    expect(nomFichier('goutte', 'neutre', 'encre', 'png')).toBe('bobby-goutte-neutre-encre.png')
    expect(nomFichier('cercle', 'hilare', 'violet', 'svg')).toBe('bobby-cercle-hilare-violet.svg')
  })
  ...
  it('translittere les accents et separe les mots', () => {
    expect(nomFichier('Cycle par défaut', '', '', 'mp4')).toBe('bobby-cycle-par-defaut.mp4')
    expect(nomFichier('Été 2026', '', '', 'gif')).toBe('bobby-ete-2026.gif')
  })

  it('survit a des ids vides', () => {
    expect(nomFichier('', '', '', 'png')).toBe('bobby.png')
  })
})
