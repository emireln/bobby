...
describe('catalogue des expressions', () => {
  it('expose 24 expressions aux identifiants uniques', () => {
    expect(EXPRESSIONS).toHaveLength(24)
    expect(new Set(EXPRESSIONS.map((e) => e.id)).size).toBe(24)
    expect(EXPRESSION_BY_ID.size).toBe(24)
  })
  ...
  it('garde les deux yeux dans la silhouette, sur les 24 expressions', () => {
    for (const e of EXPRESSIONS) {
